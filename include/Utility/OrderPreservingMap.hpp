...
class OrderPreservingMap
{
  ...
  public:
    ...
    const TKey &GetKeyByIndex(size_t index) const;
...
template <typename TKey, typename TValue>
void OrderPreservingMap<TKey, TValue>::AddItem(const TKey &key, const TValue &item)
{
    if (indexMap.find(key) != indexMap.end())
    {
        throw std::invalid_argument("The name already exists in the OrderPreservingMap.");
    }
    else
    {
...
TValue OrderPreservingMap<TKey, TValue>::GetItemByKey(const TKey &key) const
{
    ...
    {
        throw std::invalid_argument("The name doesn't exist in the OrderPreservingMap.");
    }
}
...
template <typename TKey, typename TValue>
const TKey &OrderPreservingMap<TKey, TValue>::GetKeyByIndex(size_t index) const
{
...
template <typename TKey, typename TValue>
const std::vector<TValue> &OrderPreservingMap<TKey, TValue>::GetAllItems() const
{
    return items;
}
