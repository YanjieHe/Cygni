...
class NameLocator : public ExpressionVisitor<void, Scope<NameInfo> *>
{
  ...
  public:
    ...
    const NameInfo &GetNameInfo(const Expression *node, LocationKind locationKind) const
    {
    ...
    void VisitBinary(const BinaryExpression *node, Scope<NameInfo> *scope) override;
    void VisitUnary(const UnaryExpression *node, Scope<NameInfo> *scope) override;
    void VisitConstant(const ConstantExpression *node, Scope<NameInfo> *scope) override;
    void VisitParameter(const ParameterExpression *node, Scope<NameInfo> *scope) override;
    void VisitBlock(const BlockExpression *node, Scope<NameInfo> *parent) override;
    void VisitConditional(const ConditionalExpression *node, Scope<NameInfo> *scope) override;
    void VisitCall(const CallExpression *node, Scope<NameInfo> *scope) override;
    void VisitLambda(const LambdaExpression *node, Scope<NameInfo> *parent) override;
    void VisitWhileLoop(const WhileLoopExpression *node, Scope<NameInfo> *parent) override;
    void VisitDefault(const DefaultExpression *node, Scope<NameInfo> *scope) override;
    void VisitVariableDeclaration(const VariableDeclarationExpression *node, Scope<NameInfo> *scope) override;
    void VisitNew(const NewExpression *node, Scope<NameInfo> *scope) override;
    void VisitMember(const MemberExpression *node, Scope<NameInfo> *scope) override;
