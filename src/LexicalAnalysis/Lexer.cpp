...
Lexer::Lexer(std::shared_ptr<SourceCodeFile> sourceCodeFile, const std::u32string &code)
    : sourceCodeFile{sourceCodeFile}, code{code}, line{0}, column{0}, offset{0}
{
}
...
Token Lexer::ReadIdentifier()
{
    ...
    {
        return Token(sourceCodeFile, line, column, Token::IdentifyKeyword(TokenTag::Identifier, builder), builder);
    }
}
...
void Lexer::SkipComment(std::vector<Token> &tokens)
{
    ...
    {
        tokens.push_back(Token(sourceCodeFile, line, column, TokenTag::Divide, U"/"));
    }
}
